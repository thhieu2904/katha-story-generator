...
export function DictionaryPage() {
  ...
  // Stable ref for voice — lets toggleVoice avoid re-creating on every voice state change,
  // which would force all 24 cards to re-render on every audio status change.
  ...
  // toggleVoice reads voiceRef instead of capturing `voice` in closure —
  // so this callback stays stable across voice state changes and avoids
  // forcing all 24 cards to re-render when one card starts playing.
  const toggleVoice = useCallback(async (entry: DictionaryEntry) => {
    ...
    if (currentVoice.entryId === entry.id && currentVoice.status === 'playing') {
      audio.pause();
      setVoice({ entryId: entry.id, status: 'idle' });
      return;
    }
    if (currentVoice.entryId === entry.id && currentVoice.status === 'loading') {
      audioRequestRef.current?.abort();
      setVoice({ entryId: entry.id, status: 'idle' });
      return;
    }
  ...
  }, [playbackRate]); // no `voice` dependency — reads stable voiceRef
    ...
    <div className="min-h-screen">
      <main className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          ...
          <section
            ref={resultsRef}
            className="katha-dictionary-reveal mt-8 scroll-mt-20"
            data-delay="2"
            aria-busy={loading}
          >
            ...
            {/* Initial skeleton — only shown before any data arrives */}
            {!error && loading && !data ? (
            ...
            {/* Grid keyed only by query — page changes DON'T unmount/remount cards.
                Opacity fades smoothly to signal loading without destroying the layout. */}
            {!error && data?.items.length ? (
              <div
                key={query}
                className="grid gap-2.5 transition-opacity duration-200 md:grid-cols-2 lg:grid-cols-3"
                style={{ opacity: loading ? 0.45 : 1, pointerEvents: loading ? 'none' : undefined }}
              >
